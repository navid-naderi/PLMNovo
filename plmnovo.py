"""The command line entry point for PLMNovo."""
...
class _SharedParams(click.RichCommand):
    """Options shared between most PLMNovo commands"""
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.params += [
            click.Option(
                ("-m", "--model"),
                help="""
                The model weights (.ckpt file). If not provided, PLMNovo
...
def evaluate(
    ...
    print("config", config)
    print("calculate_precision", config.calculate_precision)
    with ModelRunner(config, model) as runner:
...
def train(
    ...
    """Train a PLMNovo model on your own data.
...
@click.option(
    ...
    default="PLMNovo.yaml",
    type=click.Path(dir_okay=False),
)
def configure(output: str) -> None:
    """Generate a PLMNovo configuration file to customize.

    The PLMNovo configuration file is in the YAML format.
    """
...
def setup_logging(
    ...
    """
    if output is None:
        output = f"PLMNovo_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}"
...
def setup_model(
    ...
    """Setup PLMNovo for most commands.
    ...
    if model is None and not is_train:
        logger.error("PLMNovo model should be specificed. Terminating ...")
        raise Exception
