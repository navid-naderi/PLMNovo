...
sys.path.append("..")
from dataprocessing import ms_io
import config
...
class Spec2Pep(pl.LightningModule, ModelMixin):
    """
    A PLM-guided Transformer model for de novo peptide sequencing.
    ...
    num_training_samples: int,
        Number of training samples used to set up the alignment dual variables.
    plm_name: str
        Name of the PLM (current supported options: esm2_8m or esm2_650m).
    pooling_fn: str
        Pooling function for the peptide and spectrum embeddings (current supported option: avg).
    plm_lora_rank: int
        LoRA Rank for fine-tuning the PLM (should be a positive integer; see below for no fine-tuning)
    plm_lora_alpha: float
        LoRA Alpha for fine-tuning the PLM (use 0 for no fine-tuning).
    dual_lr: float
        Dual learning rate.
    alignment_constraint_eps: float
        Alignment constraint upper bound.
    **kwargs : Dict
    ...
    def __init__(
        ...
        plm_name: str = 'esm2_8m',
        pooling_fn: str = 'avg',
        plm_lora_rank: int = 2,
        plm_lora_alpha: float = 8,
        dual_lr: float = 0.01,
        alignment_constraint_eps: float = 0.01,
        **kwargs: Dict,
    ):
        super().__init__()
        ...
        if plm_name == 'esm2_8m':
            # ESM-2 8M
            self.plm_tokenizer = AutoTokenizer.from_pretrained("facebook/esm2_t6_8M_UR50D")
            self.plm = EsmModel.from_pretrained("facebook/esm2_t6_8M_UR50D")
            self.dim_plm = 320
            peft_config = LoraConfig(r=plm_lora_rank, lora_alpha=plm_lora_alpha, bias="none", target_modules=["key","value"], layers_to_transform=[3, 4, 5])
        elif plm_name == 'esm2_650m':
            # ESM-2 650M
            self.plm_tokenizer = AutoTokenizer.from_pretrained("facebook/esm2_t33_650M_UR50D")
            self.plm = EsmModel.from_pretrained("facebook/esm2_t33_650M_UR50D")
            self.dim_plm = 1280
            peft_config = LoraConfig(r=plm_lora_rank, lora_alpha=plm_lora_alpha, bias="none", target_modules=["key","value"], layers_to_transform=[25, 26, 27, 28, 29, 30, 31, 32])
        else:
            logger.error('PLM {} not supported! Terminating ...'.format(plm_name))
            raise Exception
        
        self.plm = inject_adapter_in_model(peft_config, self.plm)

        # ensure gradient tracking is only enabled for LoRA adapters and disabled for the rest of PLM parameters
        ...
        self.plm_proj = torch.nn.Linear(self.dim_plm, dim_model)

        self.pooling_fn = pooling_fn

        if self.pooling_fn != 'avg':
            logger.error('Pooling {} not supported! Terminating ...'.format(pooling_fn))
            raise Exception

        self.lambda_alignment = torch.zeros(num_training_samples, requires_grad=False) # initialize the dual variables
        self.alignment_constraint_eps = float(alignment_constraint_eps)
    ...
    def _forward_step(
        ...
        idxs : torch.Tensor of size (n_spectra, 1)
            A list of indices of the training samples.
        ...
        memories, mem_masks, idxs
        """

        memories, mem_masks = self.encoder(spectra)

        return self.decoder(sequences, precursors, memories, mem_masks), memories, mem_masks, idxs


    def to_unmodified(self, seq: str) -> str:
        # remove PTMs for input to the PLM and keep only uppercase letters (AAs)
        return re.sub(r'[^A-Z]', '', seq)

    def alignment_loss_calc(self, pooled_plm_embeddings, pooled_spectra_embeddings):

        # map pooled PLM embeddings to the same dimension as the spectra embeddings
        projected_plm_embeddings = self.plm_proj(pooled_plm_embeddings)
        projected_spectra_embeddings = pooled_spectra_embeddings

        normalized_plm_embeddings = torch.nn.functional.normalize(projected_plm_embeddings, dim=1)
        normalized_spectra_embeddings = torch.nn.functional.normalize(projected_spectra_embeddings, dim=1)

        loss = torch.sum(self.mse_loss_fn(normalized_plm_embeddings, normalized_spectra_embeddings), dim=1)

        return loss

    
    def pooling(self, x, mask):
        # x and mask are both B x Lmax x D
        if self.pooling_fn == 'avg':
            pooled_x = torch.sum(x * mask, dim=1) / torch.sum(mask, dim=1)
        else:
            logger.error('Pooling {} not supported! Terminating ...'.format(self.pooling_fn))
            raise Exception

        return pooled_x


    def pool_embeddings(self, plm_embeddings, plm_mask, spectra_embeddings, spectra_mask):

        pooled_plm_embeddings = self.pooling(plm_embeddings, plm_mask) # B x D'
        pooled_spectra_embeddings = self.pooling(spectra_embeddings, spectra_mask) # B x D

        return pooled_plm_embeddings, pooled_spectra_embeddings
    ...
    def training_step(
        ...
        *args,
        ...
        """
        (pred, truth), memories, mem_masks, idxs = self._forward_step(*batch)
        pred = pred[:, :-1, :].reshape(-1, self.decoder.vocab_size + 1)
        if mode == "train":
            loss = self.celoss(pred, truth.flatten())
        else:
            loss = self.val_celoss(pred, truth.flatten())
            print(f"Validation Batch CE Loss: {loss.detach():.5f}")
        ...
        # Peptide-spectrum embedding alignment
        
        # get spectrum embeddings
        spectra_embeddings = memories[:, 1:] # first one is not a peak embedding, so let's ignore it
        spectra_mask = (~mem_masks[:, 1:]).unsqueeze(-1).repeat(1, 1, memories.shape[-1])

        # get peptide embeddings
        self.plm.to(self.device)
        peptides_true_modified = list(batch[2])
        peptides_true = [self.to_unmodified(seq) for seq in peptides_true_modified] # remove PTMs for the PLM processing
        plm_inputs = self.plm_tokenizer(list(peptides_true), padding=True, return_tensors="pt").to(self.device)
        plm_outputs = self.plm(**plm_inputs)
        plm_embeddings = plm_outputs.last_hidden_state[:, 1:-1] # B x Lmax x D
        plm_mask = torch.zeros_like(plm_embeddings)
        for b in range(len(plm_embeddings)):
            plm_mask[b, :len(peptides_true[b])] = 1

        # pool the spectrum and peptide embeddings
        pooled_plm_embeddings, pooled_spectra_embeddings = self.pool_embeddings(plm_embeddings, plm_mask, spectra_embeddings, spectra_mask)

        # Calculate the alignment loss (Euclidean distance between peptide and spectrum embeddings)
        plm_alignment_loss = self.alignment_loss_calc(pooled_plm_embeddings, pooled_spectra_embeddings)

        self.log(
            f"{mode}_AlignmentMSELoss",
            plm_alignment_loss.mean().detach(),
            on_step=True,
            on_epoch=True,
            sync_dist=True,
        )

        # Dual ascent step for updating the Lagrange multipliers (lambdas) corresponding to the PSMs in the current batch
        lambda_batch_detached = self.lambda_alignment.to(self.device)[idxs].clone().detach() # detach here for use in the Lagrangian
        constraint_slacks = (plm_alignment_loss - self.alignment_constraint_eps)
        self.lambda_alignment[idxs.cpu()] = self.lambda_alignment[idxs.cpu()] + self.dual_lr * constraint_slacks.detach().cpu()
        self.lambda_alignment.data.clamp_(0) # ensure non-negativity

        self.log(
            f"{mode}_Lambdas",
            self.lambda_alignment.mean().detach(),
            on_step=True,
            on_epoch=True,
            sync_dist=True,
        )
        ...
        return loss + torch.mean(lambda_batch_detached * constraint_slacks)
    ...
    def validation_step(
        ...
        peptide_scores, peptides_pred, peptides_true = [], [], batch[2]
        for spectrum_preds in self.forward(batch[0], batch[1]):
            for score, _, pred in spectrum_preds:
        ...
        log_args = dict(on_step=True, on_epoch=True, sync_dist=True)
        self.log(
    ...
    def on_train_epoch_end(self) -> None:
        ...
        train_alignment_loss = self.trainer.callback_metrics["train_AlignmentMSELoss"].detach()
        train_lambdas = self.trainer.callback_metrics["train_Lambdas"].detach()
        metrics = {
            "step": self.trainer.global_step,
            "train": train_loss.item(),
            "train_alignment_loss": train_alignment_loss.item(),
            "train_lambdas": train_lambdas.item(),
        }
    ...
    def on_load_checkpoint(self, checkpoint):
        ...
        # if epsilon is very large, then do not reload the PLM, since the training was effectively unconstrained
        if self.alignment_constraint_eps > 1e3:
        
            # Remove parameters containing "plm"
            keys_to_remove = [k for k in state_dict.keys() if "plm" in k]
            for key in keys_to_remove:
                del state_dict[key]
        
            print(f"Removed {len(keys_to_remove)} parameters containing 'plm': {keys_to_remove}")

        checkpoint["state_dict"] = state_dict

    def load_state_dict(self, state_dict, strict=True):
        # Use strict=False to ignore missing plm parameters (in the case of unconstrained learning with large epsilon)
