...
class ModelRunner:
    """A class to run PLMNovo models.

    Parameters
    ----------
    config : Config object
        The PLMNovo configuration.
    ...
    def __init__(
        ...
        # Create a run name based on important PLMNovo hyperparameters
        run_name = "{0}_plm_lora_rank_{1}_dual_lr_{2}_pooling_fn_{3}_alignment_constraint_eps_{4}".format(config.plm_name,
        config.plm_lora_rank,
        config.dual_lr,
        config.pooling_fn,
        config.alignment_constraint_eps,
        )

        self.run_name = run_name

        WANDB_PROJ_NAME = config.wandb_proj_name

        config.model_save_folder_path += (run_name + '/')
        ...
        self.wandb_logger = WandbLogger(project=WANDB_PROJ_NAME, config=config, name=run_name)#config.save_name)#"PLMNovoplm_esm8M_contrastivecoeff.1")#_{}".format(config.random_seed))
    ...
    def evaluate(self, peak_path: Iterable[str]) -> None:
        """Evaluate peptide sequence preditions from a trained PLMNovo model.
        ...
        torch.save(self.model.all_test_results, save_path)

    def predict(self, peak_path: Iterable[str], output: str) -> None:
        """Predict peptide sequences with a trained PLMNovo model.
    ...
    def initialize_trainer(self, train: bool) -> None:
        ...
        trainer_cfg = dict(
            ...
            logger=self.wandb_logger if train else None,
        )
    ...
    def initialize_model(self, train: bool) -> None:
        ...
        # get the number of training samples for setting up the dual multiplier vector
        if train:
            num_training_samples = len(self.loaders.train_dataset)
        else:
        ...
        model_params = dict(
            ...
            plm_name=self.config.plm_name,
            pooling_fn=self.config.pooling_fn,
            plm_lora_rank=self.config.plm_lora_rank,
            plm_lora_alpha=self.config.plm_lora_alpha,
            dual_lr=self.config.dual_lr,
            alignment_constraint_eps=self.config.alignment_constraint_eps,
        )
